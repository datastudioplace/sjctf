...
import com.localhost8888.helperutilities.*;
import com.localhost8888.restcalls.HTTPClientPost;
import com.localhost8888.restcalls.HTTPClientPut;
...
public class HTTPAPIRequestStepDefs {
  ...
  public void userSetup() throws IOException, JSONException {
    ...
    JSONObject jsonObjectChangePassword =
        new JSONObject(
            "{\n"
    ...
    httpResponse =
        clientPut.httpPutClient(
            GlobalVarsHelper.getInstance().getURL()
                + "administrator-gateway/administrator-auth/api/administrator/account/password",
            jsonObjectChangePassword);
    if (httpResponse.getStatusLine().getStatusCode() == 200) {
      Assert.assertEquals(200, clientPut.getResponseCode());
    } else {
      //			Assert.assertEquals("Something has gone wrong", 400,
      // httpResponse.getStatusLine().getStatusCode());
    }

    // Authentication of the admin user
    processFileResouce =
        new ProcessFileResouce(
                "com/localhost8888/testData/testData/jsons/storydatafile.json");
  ...
  public void iUseRequestMethod(String arg0) throws Throwable {
    if (arg0.toLowerCase().contains("post")) bPost = true;
    else bPost = false;
  }
  ...
  public void theResponseShouldBeAnd(String arg0, String arg1) throws Throwable {
    if (bPost) {
      clientPost =
          new HTTPClientPost(GlobalVarsHelper.getInstance().getURL() + endpoint, jsonObject);
      Assert.assertEquals(arg0, String.valueOf(clientPost.getResponseCode()));
    }
  }

  @Given(
      "^The \"([^\"]*)\" password needs changing using the initial password \"([^\"]*)\" using endpoint \"([^\"]*)\"$")
  public void thePasswordNeedsChangingUsingTheInitialPasswordUsingEndpoint(
      String arg0, String arg1, String arg2) throws Throwable {
  ...
  public void iChangeThePasswordTo(String arg0) throws Throwable {
    ...
    httpResponse =
        clientPut.httpPutClient(
            GlobalVarsHelper.getInstance().getURL() + endpoint, jsonObjectChangePassword);
  }
  ...
  public void theUserIsReady() throws Throwable {
    if (httpResponse.getStatusLine().getStatusCode() == 200) {
      ...
      Assert.assertEquals(
          400, httpResponse.getStatusLine().getStatusCode(), "Something has gone wrong");
    }
  }
  ...
  public void clearTestData() throws Throwable {
    ...
    vacancyConnect.setConnection(
        data.getVacancyDatabase(),
        data.getDatabaseusername(),
        data.getDatabasepassword(),
        data.getUrl());
    ...
    jobAdvertConnect.setConnection(
        data.getJobadvertDatabase(),
        data.getDatabaseusername(),
        data.getDatabasepassword(),
        data.getUrl());
  ...
  private DataSettings loadDatabaseDetails() throws IOException {
    ...
    Yaml yaml = new Yaml();
    InputStream in =
        Connect.class.getResourceAsStream(
                "/com/localhost8888/testData/testData/jsons/application.yml");
